    ...
    if request.method == 'POST':
        ...
        if db.add_patient(iD, name, age, gender, email, contact, address):
            if db.add_auth(iD, pwd):
                app.logger.info(
                    f"Patient {iD} registered for clinical exam at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
                image.save(os.path.join(
                    app.config['IMAGE_UPLOADS'], f'{iD}.png'))
...
# @app.route('/add_me')
# def admin():
# 	db.add_staff('Admin', 'Dr', 'admin', '0241234567')
# 	db.add_auth('admin', 'admin')
...
def home(session, staff):
    ...
    app.logger.info(
        f"Staff {data['inf']} logged in at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
    ...
    if request.method == 'POST':
        ...
        if db.validate_patient(iD):
            if db.add_patient_wh(iD, weight, height):
                data['msg'] = 'success'
                app.logger.info(
                    f"Staff {data['inf']} added data for patient {iD} at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
    ...
    if request.method == 'POST':
        ...
        if db.validate_patient(iD):
            if db.add_x_ray(iD, remark):
                ...
                image.save(os.path.join(
                    app.config['IMAGE_UPLOADS'], f'{iD}_x_ray.png'))
                data['msg'] = 'success'
                app.logger.info(
                    f"Staff {data['inf']} added data for patient {iD} at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
...
def login_student():
    ...
    if request.method == 'POST':
        ...
        if db.validate_login(iD, passw):
            app.logger.info(
                f"Patient {iD} logged in at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")

            return redirect(f'/patient_check/{iD}/{session}')
...
@app.route('/patient/<patient>/<session>/<staff>', methods=['GET', 'POST'])
def patient(patient, session, staff):
    data = {
        "session": session,
        "patient": db.retrieve_patient_data(patient),
        "info": db.retrieve_staff_data(staff),
        "inf": staff
    }
    ...
    if request.method == 'POST':
        remark = request.form.get('remark')
        if db.add_remark(patient, remark):
            data['msg'] = 'success'
...
@app.route('/patient_check/<patient>/<session>/<staff>')
def patients(patient, session, staff):
...
@app.route('/patient_check/<patient>/<session>')
def patient_page(patient, session):
    data = {
        "session": session,
        "patient": db.retrieve_patient_data(patient),
        "info": db.retrieve_staff_data(staff),
    ...
    if request.method == "POST":
        ...
        if db.add_staff(name, title, staff_id, contact):
            if db.add_auth(staff_id, pwd):
                ...
                image.save(os.path.join(
                    app.config['IMAGE_UPLOADS'], f'{staff_id}.png'))
                data['msg'] = 'success'
...
def add_appointment(session, staff):
    ...
    if request.method == 'POST':
        ...
        if db.validate_staff(officer):
            if db.validate_patient(patient):
                if db.add_appointment(patient, officer, date, time):
                    data['msg'] = 'success'
                    app.logger.info(
                        f"Staff {data['inf']} booked appointment for patient {patient} at {datetime.datetime.today()} on {date} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
                ...
                if db.validate_batch(request.form.get('batch')):
                    if db.add_prescription(patient, staff, name):
                        if db.update_stock(batch) != 'empty':
                            data['msg'] = 'success'
                            app.logger.info(
                                f"Staff {data['inf']} added prescribed {name} for patient {iD} at {datetime.datetime.today()} from IP {request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}")
...
def add_patient(session, staff):
    ...
    if request.method == "POST":
        ...
        if db.add_patient(patient_id, name, age, gender, email, contact, address):
            image.save(os.path.join(
                app.config['IMAGE_UPLOADS'], f'{patient_id}.png'))
